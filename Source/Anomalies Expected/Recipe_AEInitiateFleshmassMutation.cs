    ...
    public class Recipe_AEInitiateFleshmassMutation : Recipe_Surgery
    {
        public override IEnumerable<BodyPartRecord> GetPartsToApplyOn(Pawn pawn, RecipeDef recipe)
        {
            return MedicalRecipesUtility.GetFixedPartsToApplyOn(recipe, pawn, delegate (BodyPartRecord record)
            {
                ...
                return (!pawn.health.hediffSet.hediffs.Any((Hediff x) => x.Part == record && (x.def == recipe.addsHediff || (x.def == mutation && ((x as HediffWithComps).GetComp<HediffComp_FleshmassMutation>()?.hediffToAdd ?? null) == recipe.addsHediff) || !recipe.CompatibleWithHediff(x.def)))) ? true : false;
            });
        }
